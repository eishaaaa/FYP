...
class BlockchainServiceEnhanced {
  ...
  /// NEW: Transfer Electronic Asset (ERC-721)
  Future<String?> transferElectronic({
    ...
    // safeTransferFrom has an overload; usually we use: from, to, tokenId
    // Note: In Web3Dart with Overloads, you might need to be specific, but standard call matches signature
    final transaction = Transaction.callContract(
      contract: _electronicsContract,
      function: function,
      parameters: [
        EthereumAddress.fromHex(connectedAddress!), // From (Current Owner)
        EthereumAddress.fromHex(toAddress),         // To (Receiver)
        BigInt.from(tokenId),                       // Token ID
      ],
    );
    return await _sendTransaction(transaction);
  }
  ...
  /// NEW: Transfer Land Fractions (ERC-1155)
  Future<String?> transferLandFraction({
    ...
    // ERC-1155 safeTransferFrom args: from, to, id, amount, data
    final transaction = Transaction.callContract(
      contract: _landContract,
      function: function,
      parameters: [
        EthereumAddress.fromHex(connectedAddress!), // From
        EthereumAddress.fromHex(toAddress),         // To
        BigInt.from(propertyId),                    // Property ID
        BigInt.from(amount),                        // Amount of fractions
        Uint8List(0),                               // Data (empty bytes)
      ],
    );
    return await _sendTransaction(transaction);
  }
